...
class HandleDBMSMySQL {
  constructor(host = null, database = null, user = null, password = null) {
    var session_file = JSON.parse(
      fs.readFileSync("./config/server/env.json", "utf8", "r")
    );
    if (session_file) {
      this._host =
        typeof host !== "string" || host == null ? session_file.host : host;
      this._database =
        typeof database !== "string" || database == null
          ? session_file.database
          : database;
      this._user =
        typeof user !== "string" || user == null ? session_file.user : user;
      this._password =
      ...
      console.error(
        "Parâmetros incorretos para a classe: `%s`",
